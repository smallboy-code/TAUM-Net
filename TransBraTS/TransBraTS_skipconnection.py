...
class MGMT_Feature_Attention(nn.Module):
    def __init__(self):
        super(MGMT_Feature_Attention, self).__init__()
        ...
        self.hidder_layer_0 = nn.Linear(1408, 1280)
        self.hidder_layer_1 = nn.Linear(1280, 512)
        self.hidder_layer_2 = nn.Linear(512, 32)
        self.hidder_layer_3 = nn.Linear(256, 64)
        self.hidder_layer_4 = nn.Linear(64, 32)
        self.drop_layer = nn.Dropout(p=0.2)
        self.classifier = nn.Linear(32, 2)
...
class mgmt_network(nn.Module):
    def __init__(self):
        super(mgmt_network, self).__init__()
        # self.avg_pool_3d = nn.AvgPool3d(16, 1)
        # self.max_pool_3d = nn.MaxPool3d(16, 1)
        # self.Hidder_layer_1 = nn.Linear(1280, 512)
        # self.Hidder_layer_2 = nn.Linear(512, 32)
        ...
        self.avg_pool_3d = nn.AvgPool3d(16, 1)
        self.max_pool_3d = nn.MaxPool3d(16, 1)
        self.avg_pool_3d_1 = nn.AvgPool3d(32, 1)
        self.max_pool_3d_1 = nn.MaxPool3d(32, 1)
        self.Hidder_layer_1 = nn.Linear(1408, 512)
    ...
    def forward(self, x4_1, x3_1, encoder_output):
        x = self.feature_fusion_layer(x4_1,x3_1,encoder_output)
        x = self.drop_layer(x)
        x = self.Hidder_layer_1(x)
        x = self.Hidder_layer_2(x)
        x = self.Hidder_layer_3(x)
        x = self.Hidder_layer_4(x)
        y = self.classifier(x)
        return y

    def feature_fusion_layer(self, x4_1, x3_1, encoder_output):
        x4_1_avg = self.avg_pool_3d(x4_1)
        x4_1_max = self.max_pool_3d(x4_1)

        x3_1_avg = self.avg_pool_3d_1(x3_1)
        x3_1_max = self.max_pool_3d_1(x3_1)
        ...
        x3_1_avg = x3_1_avg.view(x3_1_avg.size(0), -1)
        x3_1_max = x3_1_max.view(x3_1_max.size(0), -1)

        encoder_avg = encoder_avg.view(encoder_avg.size(0), -1)
        encoder_avg_max = encoder_avg_max.view(encoder_avg_max.size(0), -1)

        return torch.cat([x3_1_avg, x3_1_max, x4_1_avg,x4_1_max,encoder_avg,encoder_avg_max], dim=1)
...
class TransformerBraTS(nn.Module):
    ...
    def forward(self, x):
        ...
        # MGMT_out = self.MGMT_classifier(x4_1, encoder_output,y4_1)
        ...
        #
        #    return MGMT_out,decoder_output
        #
        # return MGMT_out,decoder_output
    ...
    # def MGMT_classifier(self,x4_1, encoder_output,y4_1 ):
    #     """
    #
