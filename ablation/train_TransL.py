...
def main_worker():
    if args.local_rank == 0:
        log_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'logTransL' + args.date)
    ...
    model = TransBraTS(dataset='brats', _conv_repr=True, _pe_type="learned", num_layers=3)#修改num_layers,即可改变Transformer层数
    ...
    if args.local_rank == 0:
        checkpoint_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'checkpoint_TransL',
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.local_rank == 0:
                logging.info(
                    'Epoch: {}_Iter:{}  loss: {:.5f} seg_loss: {:.5f} mgmt_loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 3:{:.4f} ||  seg_std:{:.4f} mgmt_std:{:.4f} seg_vars:{:.4f} mgmt_vars:{:.4f}'
                    .format(epoch, i, reduce_loss, reduce_seg_loss, reduce_mgmt_loss, reduce_loss1, reduce_loss2,
                            reduce_loss3, seg_std, mgmt_std, seg_vars, mgmt_vars))
            loss.backward()
            optimizer.step()
            ...
            if args.local_rank == 0:
                ...
                if min_loss >= epoch_valid_loss:
                    ...
                    torch.save({
                        ...
                        file_name)
                logging.info(
                    'Epoch:{}[best_epoch:{} min_total_loss:{:.5f} ||epoch_valid_loss:{:.5f} | seg_loss: {:.5f} | mgmt_loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 3:{:.4f} | mgmt_acc: {:.5f} | mgmt_auc:{:.5f}||'
                    .format(epoch, best_epoch, min_loss, epoch_valid_loss, epoch_seg_loss, epoch_mgmt_loss,
                            epoch_dice_1, epoch_dice_2, epoch_dice_3, accuracy, auc))
