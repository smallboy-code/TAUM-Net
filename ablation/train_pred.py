...
from models.criterions import MultiTaskLossWrapper, FocalLoss_seg, mgmt_cross_entropy
...
def main_worker():
    if args.local_rank == 0:
        log_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'logPred' + args.date)
    ...
    if args.local_rank == 0:
        checkpoint_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'checkpoint_seg',
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            mgmt_loss = mgmt_cross_entropy(mgmt_out, mgmt)
        ...
        with torch.no_grad():
            ...
            if args.local_rank == 0:
                ...
                if min_loss >= epoch_mgmt_loss:
                    min_loss = epoch_mgmt_loss
                    best_epoch = epoch
                    ...
                    torch.save({
                        ...
                        file_name)
                logging.info(
                    'Epoch:{}[best_epoch:{} min_mgmt_loss:{:.5f} acc:{:.5f} auc:{:.5f}'
                    .format(epoch, best_epoch, epoch_mgmt_loss, accuracy, auc))
