...
from predict import validate_softmax_mgmt
...
parser.add_argument('--root', default='/opt/data/private/Brats_Dataset/', type=str)

# parser.add_argument('--valid_dir', default='MICCAI_BraTS2020_ValidationData', type=str)

parser.add_argument('--valid_file', default='mgmt_test.txt', type=str)
...
parser.add_argument('--experiment', default='TransBraTS_mgmt', type=str) #TransBraTS

parser.add_argument('--test_date', default='2024-01-29', type=str)
...
def main():
    ...
    mgmt_model = torch.nn.DataParallel(mgmt_model).cuda()
    # Grade_model =torch.nn.DataParallel(Grade_model).cuda()
    #only_seg=

    # dict_model = {'en':model,'seg':seg_model,'mgmt':mgmt_model,'grade':Grade_model}
    dict_model = {'en': model, 'mgmt': mgmt_model}
    ...
    if os.path.exists(load_file):
        ...
        dict_model['mgmt'].load_state_dict(checkpoint['mgmt_state_dict'])
    ...
    # valid_list2 = 'data/mgmt_test.csv'
    # valid_root2 = '/opt/data/private/Brats_Dataset/'
    # valid_set2 = BraTS(valid_list, valid_root, mode='test')
    # print('Samples for valid = {}'.format(len(valid_set)))
    ...
    with torch.no_grad():
        validate_softmax_mgmt(valid_loader=valid_loader,
                         model=dict_model,
                         load_file=load_file,
                         multimodel=False,
                         # savepath=submission,
