...
#mgmt_cross_entropy = F.cross_entropy
...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, outputs,targets,weights):
        ...
        mgmt_loss = self.loss_fn[1](outputs[1], targets[1], weights[1])

        mgmt_loss_1 = torch.sum(0.5 * torch.exp(-self.log_vars[1]) * mgmt_loss + self.log_vars[1],-1)

        loss = torch.mean(seg_loss_1+mgmt_loss_1)

        return loss,seg_loss,mgmt_loss,loss1,loss2,loss3,std_1,std_2,self.log_vars[0],self.log_vars[1]
...
class MultiTaskLossWrapper_seg(nn.Module):
    ...
    def forward(self, outputs,targets,weights):
        ...
        loss = torch.mean(seg_loss_1)

        return loss,seg_loss,loss1,loss2,loss3,std_1,std_2,self.log_vars[0],self.log_vars[1]
...
class MultiTaskLossWrapper_1(nn.Module):
    ...
    def forward(self, outputs,targets,weights):
        ...
        mgmt_loss_1 = torch.sum(0.5 / (self.params[1]**2) * mgmt_loss + torch.log(self.params[1]**2),-1)

        loss = seg_loss_1+mgmt_loss_1

        return loss,seg_loss,mgmt_loss,loss1,loss2,loss3,std_1,std_2,self.params[0]**2,self.params[1]**2
...
class MultiTaskLossWrapper1(nn.Module):
    ...
    def forward(self, outputs,targets,weights):
        ...
        mgmt_loss_1 = torch.sum(0.5 * mgmt_loss /(self.vars[1]**2) + torch.log(self.vars[1]),-1)

        loss = torch.mean(seg_loss_1 + mgmt_loss_1)

        return loss,seg_loss,mgmt_loss,loss1,loss2,loss3,self.vars[0],self.vars[1]
...
def mgmt_cross_entropy(input,target,weight):
...
def mgmt_focal_loss(input,target,weight):
