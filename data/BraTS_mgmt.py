...
class BraTS(Dataset):
    def __init__(self, list_file, root='', mode='train', csv_file='addition_data.csv'):
        ...
        #         names.append(row[0])
        #         mgmts.append(int(row[1]))
        #         path = os.path.join(root, row[0], row[0] + '_')
        #         paths.append(path)
        #         self.lines.append(line)
        ids = pd.read_csv(list_file, dtype=str)
        for index, id in ids.iterrows():
            names.append('BraTS2021_' + id['BraTS21ID'])
            mgmts.append(int(id['MGMT_value']))
            path = os.path.join(root, 'BraTS2021_Training_Data/BraTS2021_' + id['BraTS21ID'], 'BraTS2021_' + id['BraTS21ID'] + '_')
            paths.append(path)
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            ...
            if len(data) == 2:
                ...
                return sample['image'], sample['label'],torch.tensor(mgmt)
            if len(data)==3:
                ...
                if name in self.add_ids:
                    mgmt_target = self.add_data[self.add_data.id==name].mgmt_class.values.ravel()[0]

                return sample['image'], sample['label'],grade[0],mgmt_target,sample['weight']
            ...
            if len(data) == 2:
                ...
                return image, label,torch.tensor(mgmt)
            else:
                image = data[:,:,:,:]
                image = np.pad(image, ((0, 0), (0, 0), (0, 5), (0, 0)), mode='constant')
                image = np.ascontiguousarray(image.transpose(3, 0, 1, 2))
                image = torch.from_numpy(image).float()
                return image,torch.tensor(mgmt)
...
if __name__ == '__main__':
     train_list = 'mgmt_train.csv'
     train_root = 'F:/'
     mode = 'test'
     train_set = BraTS(train_list, train_root, mode)
     #data[0]:四个模态的图,data[1]:seg,data[2]:mgmt标签
