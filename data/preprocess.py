...
#todo 记得改
train_set = {
        'root': 'F:/BraTS2021_TrainingData/',
        'flist': 'mgmt_train.csv',
...
def doit(dset):
    ...
    paths = [(root + 'BraTS2021_' + str(name) + '/BraTS2021_' + str(name) + '_') for name in id]
    pbar = tqdm(paths)
    for path in pbar:
        pbar.set_description('Processing ' + path)
