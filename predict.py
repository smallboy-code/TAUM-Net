...
def tailor_and_concat(x, model):
    ...
    for i in range(len(temp)):
        if isinstance(model,dict):
            ...
            mgmt_out = model['mgmt'](encoder_outs[3],encoder_outs[2], encoder_outs[4])  #x4_1, encoder_output
            # grade_out = model['grade'](encoder_outs[3], encoder_outs[4])
            temp[i] = seg_output
            print("mgmt_out:",mgmt_out)
    ...
    if isinstance(model,dict):
        ...
        print("mgmt_out mean:",mgmt_out)
        return y[..., :155],mgmt_out#,grade_out
    else:
        return y[..., :155]


def tailor_and_concat2(x, model):
    temp = []
    mgmt_temp=[]
...
def validate_softmax(
    ...
    t1ce_Path = 'F:/BraTS2021_Training_Data/BraTS2021_00000/BraTS2021_00000_t1ce.nii.gz'
    ...
    for i, data in enumerate(valid_loader):
        ...
        if not use_TTA:
            ...
            if multimodel:
                ...
                output = F.softmax(logit[0], dim=1)
            ...
            if isinstance(model,dict):
                ...
                for pred in [TTA_1, TTA_2, TTA_3, TTA_4, TTA_5, TTA_6, TTA_7, TTA_8]:
                    ...
                    # mgmt_probs.append(pred[1].sigmoid())
                    mgmt_probs.append(F.softmax(pred[1],1))
                    # grade_probs.append(F.softmax(pred[2],1))
                # #print("mgmt_probs:",mgmt_probs)
    ...
    if isinstance(model, dict):
        ...
        import pandas as pd
        data = pd.DataFrame({"ID":ids,"pred":mgmt_prob,"pred_class":mgmt_class,"mgmt_truth":mgmt_truth})
        ...
        print("ACU:",roc_auc_score(mgmt_truth,mgmt_prob))
...
def validate_softmax_seg(
    ...
    t1ce_Path = '/opt/data/private/Brats_Dataset/BraTS2021_Training_Data/BraTS2021_00000/BraTS2021_00000_t1ce.nii.gz'
    t1ce_image = nib.load(t1ce_Path)
    H, W, T = 240, 240, 155
    if isinstance(model,dict):
        model['en'].eval()
