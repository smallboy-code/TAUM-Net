...
parser.add_argument('--experiment', default='TransBraTS_mgmt', type=str)
...
def main_worker():
    ...
    DDP_model = {
        ...
        'seg': nn.parallel.DistributedDataParallel(nets['seg'], device_ids=[args.local_rank], output_device=args.local_rank,
                                                find_unused_parameters=True),
        'mgmt': nn.parallel.DistributedDataParallel(nets['mgmt'], device_ids=[args.local_rank], output_device=args.local_rank,
                                                find_unused_parameters=True),
    ...
    if args.local_rank == 0:
        checkpoint_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'checkpoint_mgmt', args.experiment+args.date)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            loss,seg_loss,mgmt_loss,loss1,loss2,loss3,seg_std, mgmt_std,log_var_1,log_var_2= DDP_model['mtl']([y,mgmt_out],[target,mgmt],[None,weight])
            ...
            if args.local_rank == 0:
                logging.info('Epoch: {}_Iter:{}  loss: {:.5f} seg_loss: {:.5f} mgmt_loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 3:{:.4f} ||  seg_std:{:.4f} mgmt_std:{:.4f} seg_vars:{:.4f} mgmt_vars:{:.4f}'
                             .format(epoch, i, reduce_loss,reduce_seg_loss,reduce_mgmt_loss, reduce_loss1, reduce_loss2, reduce_loss3,seg_std,mgmt_std,seg_vars,mgmt_vars))
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(valid_loader):
                ...
                valid_loss, seg_loss, mgmt_loss, loss1, loss2, loss3,std_1,std_2,var_1, var_2 = DDP_model['mtl']([seg_out, mgmt_out], [target, mgmt],
                                                                                  [None, None])
            ...
            if args.local_rank == 0:
                ...
                if min_loss >= epoch_valid_loss:
                    ...
                    torch.save({
                        ...
                        file_name)
                logging.info('Epoch:{}[best_epoch:{} min_total_loss:{:.5f} ||epoch_valid_loss:{:.5f} | seg_loss: {:.5f} | mgmt_loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 3:{:.4f} | mgmt_acc: {:.5f} | mgmt_auc:{:.5f}||'
                             .format(epoch, best_epoch,min_loss,epoch_valid_loss, epoch_seg_loss,epoch_mgmt_loss,epoch_dice_1,epoch_dice_2,epoch_dice_3,accuracy, auc))
...
def lbp(src):
    ...
    # src = src.cpu().numpy()
    height, width = src.shape[3:5]
    # for c in range(4):
    c=3
    # hist=np.zeros(src.shape[2])
    hist=np.zeros((256,1))
    for j in range(src.shape[2]):
        src_tmp = src[0][c][j]
        src_tmp=np.around(tool.fit_transform(src_tmp)*255)


        # dst = np.zeros((c+1, j+1, height, width), dtype=np.uint8)
        dst = np.zeros((j + 1, height, width), dtype=np.uint8)

        lbp_value = np.zeros((1,8), dtype=np.uint8)
        neighbours = np.zeros((1,8), dtype=np.uint8)
        for row in range(1, height-1):
            for col in range(1, width-1):
                center = src_tmp[row, col]

                neighbours[0, 0] = src_tmp[row-1, col-1]
                neighbours[0, 1] = src_tmp[row-1, col]
                neighbours[0, 2] = src_tmp[row-1, col+1]
                neighbours[0, 3] = src_tmp[row, col+1]
                neighbours[0, 4] = src_tmp[row+1, col+1]
                neighbours[0, 5] = src_tmp[row+1, col]
                neighbours[0, 6] = src_tmp[row+1, col-1]
                neighbours[0, 7] = src_tmp[row, col-1]
                ...
                # dst[c, j, row, col] = lbp
                dst[j, row, col] = lbp
        hist +=cv2.calcHist(dst[j], [0], None, [256], [0,255])
        # print(dst.shape)
    print(hist.shape)
